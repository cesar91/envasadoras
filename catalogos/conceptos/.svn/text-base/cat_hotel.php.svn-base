...
if(isset($_GET["Eid"])){
	...
	<table style="background:#F8F8F8">
		<tr>
			<td>Hotel<span class="style1">*</span>:</td>
			<td><input type="text" name="name_hotel" id="name_hotel"/></td>
			<td align="right">Costo<span class="style1">*</span>:</td>
			<td><input type="text" name="costo" id="costo" onkeypress="return validaNum(event)" /></td>
		</tr>
		<tr>
			<td colspan="4">Ciudades:</td>
		</tr>
		<tr>
			<td align="right"><input type="radio" name="radioCd" id="radioCd" checked="checked" onclick="activeCd();"/>Existentes<span class="style1">*</span>:</td>
			<td colspan="4">
				<select id="select_cd" name="select_cd">
				<?php foreach($aux as $cd){ ?>

...
?>
<center>
	<h1>Cat&aacute;logo de Conceptos</h1>
<?php
$paginacion="";
...
<?php
			...
			$L->Cabeceras("Folio");
			$L->Cabeceras("Nombre");
			$L->Cabeceras("Clasificación");
			$L->Cabeceras("Cuenta");			
			$L->Herramientas("E","./index.php?".$paginacion."catalogo=cg&Eid=");
			$L->Herramientas("D","./index.php?".$paginacion."catalogo=cg&delete=true&id=");
			
			$query="select dc_id, cp_concepto, cp_clasificacion, cp_cuenta from cat_conceptos where cp_activo=1  and dc_catalogo=1 order by dc_id";

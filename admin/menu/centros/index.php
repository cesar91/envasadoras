...
switch($mode){
    ...
    case "INDEX": // Lista de usuarios
        ...
        $query="SELECT cc_id, cc_centrocostos, cc_nombre, (SELECT nombre FROM empleado WHERE idfwk_usuario=cc_responsable), e_codigo,if (cc_estatus=0,'Activo','Inactivo')cc_estatus FROM cat_cecos c 
INNER JOIN empresas e ON c.cc_empresa_id = e.e_id 
WHERE cc_estatus >= 0 ORDER BY cc_estatus asc,cc_centrocostos asc"; 
    ...
    case "BUSCAR": // Muestra el resultado de una busqueda 
        ...
        $query="SELECT cc_id, cc_centrocostos, cc_nombre, (SELECT nombre FROM empleado WHERE idfwk_usuario=cc_responsable), e_codigo,if (cc_estatus=0,'Activo','Inactivo')as cc_estatus FROM cat_cecos c 
				INNER JOIN empresas e ON c.cc_empresa_id = e.e_id  
				WHERE cc_estatus >= 0 
				AND (cc_centrocostos = '".$criterio."' OR cc_nombre LIKE '%".$criterio."%') 

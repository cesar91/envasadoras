...
function cargarExcepciones(tramite){
	...
	if (json == null){
		...
		for(var i = 1; i <= Objectlength(json); i++ )
			tr+= '<tr><td>'+json[i]["concepto"]+'</td><td>'+json[i]["mensaje"]+'</td><td>'+json[i]["excedente"]+'</td></tr>';			
...
function createTableJS(idTable,classTable,styleTable,th,thStyle){
	var i=0;
	var cadena ="<table id='"+idTable+"' class='"+classTable+"' "+styleTable+">";
			cadena += "<thead>";
				cadena += "<tr>";
					for(i==1;i<th.length;i++){
						cadena += "<th "+thStyle[i]+">"+th[i]+"</th>";
					}
				cadena += "</tr>";
			cadena += "</thead>";
			cadena += "<tbody>";

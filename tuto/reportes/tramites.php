...
<center><h3>Reporte Integral de Trámites</h3></center>
...
<table align='center', width='30%'>    
    ...
    <tr>
        ...
        <td colspan='1' align='left'>
           <select name="usuarilist" id="usuarilist">
                <option name="Todas" id="Todas" value="Todas">Todos</option>
               <?php      
               $usuarilist=new Usuario();
               foreach($usuarilist->Load_all() as $arrE){
               ?>
...
(SELECT e_codigo FROM empresas WHERE e_id = 
  (SELECT cc_empresa_id FROM cat_cecos WHERE cc_id = sv_ceco_paga) ) as empresa, 
   ...
   u_nombre as usuario,
 sv_tramite, sv_fecha_registro, sv_anticipo,  co_id, co_fecha_registro, co_total, co_total_aprobado, co_mi_tramite,
 co_total - coalesce(co_total_aprobado, 0) as remanente, t.t_flujo as documento, t2.t_flujo as documento2
  , (SELECT cc_centrocostos FROM cat_cecos WHERE cc_id =  co_cc_clave) as ceco_comp
 ...
 LEFT JOIN tramites as t2 on (co_mi_tramite = t2.t_id)
   WHERE sv_fecha_registro >= '%s  00:00:01' AND sv_fecha_registro <= '%s 23:59:59' %s
   
   UNION
   
SELECT  
(SELECT e_codigo FROM empresas WHERE e_id = 
  (SELECT cc_empresa_id FROM cat_cecos WHERE cc_id = co_cc_clave) ) as empresa,
   (SELECT cc_centrocostos FROM cat_cecos WHERE cc_id = co_cc_clave) as ceco,
   u_nombre as usuario, '', '', 0, co_id, co_fecha_registro, co_total, co_total_aprobado,  co_mi_tramite, 
 0 as remanente, '', t_flujo as documento2
   , (SELECT cc_centrocostos FROM cat_cecos WHERE cc_id =  co_cc_clave) as ceco_comp

FROM comprobaciones c 
INNER JOIN tramites on co_mi_tramite = t_id
 INNER JOIN usuario U on(t_iniciador=U.u_id) 
WHERE t_flujo IN (2,4,5)   
   ...
   ", $inicio, $fin, $extra);
            ...
            while($fila=mysql_fetch_assoc($rst)){
                ...
                if($fila["documento2"]=="1"){
					$fila["documento2"]="Anticipo";
				}
				if($fila["documento2"]=="2"){
					$fila["documento2"]="Amex";
				}
				if($fila["documento2"]=="3"){
					$fila["documento2"]="Comprobación";
				}
				if($fila["documento2"]=="4"){
					$fila["documento2"]="Comprobación TDC";
				}
				if($fila["documento2"]=="5"){
					$fila["documento2"]="Reembolso Caja Chica";
				}                
                
                array_push($aux,
                    array(   
                        "folio_anticipo"	=>$fila["sv_tramite"], 
                        ...
                        "tipo2"		=>$fila["documento2"],                        
                        "fecha_co"	=>$fila["co_fecha_registro"],  
                        "ceco_comp"	    =>$fila["ceco_comp"],    
...
if(isset($_POST['empresalist']) && $_POST['empresalist']!="" && 
    isset ($_POST['buscar']) && isset($_POST['fechaInicio']) && $_POST['fechaInicio']!="" && 
		...
		$aux.="<tr>";
		$aux.="<td style=\"font-size: 9px;\" align='right'><strong>Folio</strong></td>";
		...
		$resultado=Saldos_Comprobar(1,$fechaInicio,$fechaFin,$empresalist,$centro,$usuarilist,$tipo);
